...
class VAETrainer(object):
    def __init__(
        ...
        vae: DDP, vae_wo_ddp: VQVAE, disc: DDP, disc_wo_ddp: DinoDisc, ema_ratio: float,  
        dcrit: str, vae_opt: AmpOptimizer, disc_opt: AmpOptimizer,
        daug=1.0, lpips_loss=None, lp_reso=64, wei_l1=1.0, wei_l2=0.0, wei_entropy=0.0, wei_lpips=0.5, wei_disc=0.6, adapt_type=1, bcr=5.0, bcr_cut=0.5, reg=0.0, reg_every=16,
    ...
    def train_step(
        ...
        with maybe_record_function('VAE_rec'):
            ...
            if using_lpips:
                ...
                Lpip = self.lpips_loss.forward(inp, rec_B3HW)
                Lpip = Lpip.mean() 
        ...
        if warmup_disc_schedule > 0:
            with maybe_record_function('VAE_disc'):
                ...
                with self.disc_opt.amp_ctx:
                    ...
                    Lg = -self.disc_wo_ddp(self.daug.aug(rec_B3HW, fade_blur_schedule), grad_ckpt=False).mean()  
        ...
        with maybe_record_function('trainer_log'):
            ...
            if loggable:
                ...
                kw[f'z_voc_usage'] = np.array(usage).mean()
    ...
    def val_step(self,inp):
        
        with torch.no_grad():
            with self.vae_opt.amp_ctx:
                self.vae_wo_ddp.forward
                rec_B3HW, *_  = self.vae(inp)
