...
def build_things_from_args(args: arg_util.Args):
    ...
    if not args.local_debug:
        ...
        val_transform = transforms.Compose([
            transforms.Resize(round(args.img_size*1.3), interpolation=InterpolationMode.LANCZOS),   
    ...
    if not os.path.exists(vis_path):
        if dist.is_local_master():
            misc.os_system(f'mkdir -p {vis_dir}; cp /{vis_file} {vis_dir}/ >/dev/null 2>&1')
    dist.barrier()
    ...
    return (
        tb_lg, trainer,
        start_ep, start_it, acc_str, eval_milestone, iters_train, ld_train,ld_val,iters_val
    )
...
def val_one_ep( args: arg_util.Args, ld_or_itrt, iters_val: int, trainer):
    ...
    for it, inp in enumerate(ld_or_itrt):
        inp = inp.to(args.device, non_blocking=True)
        
        loss_total+=trainer.val_step(inp)
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        if is_val_and_also_saving:
            val_loss = val_one_ep(
                args, ld_val, iters_val, trainer
            )
            print(f' [*] [ep{ep}] Lm: {val_loss:.4f}')
        ...
        if dist.is_local_master():
            ...
            if is_val_and_also_saving:
                if val_loss < best_val_loss:
                    ...
                    print(f'     [saving best ckpt](*) Best val loss updated: {best_val_loss:.4f} -> {val_loss:.4f}', flush=True)
                    best_val_loss = val_loss
                else:
                    print(f'     [best ckpt not updated](*) Current val loss: {val_loss:.4f}, Best val loss: {best_val_loss:.4f}', flush=True)
