...
class LPIPS(nn.Module):
    ...
    def __init__(self, lpips_path, use_dropout=False):    
        ...
        self.lins = nn.ModuleList([NetLinLayer(c, use_dropout=use_dropout) for c in [64, 128, 256, 512, 512]])  
    ...
    def forward(self, inp, rec):
        ...
        """
        B = inp.shape[0]
        inp_and_recs = torch.cat((inp, rec), dim=0).sub(self.shift).mul_(self.scale_inv)  
        inp_and_recs = self.net(inp_and_recs)   
        diff = 0.
